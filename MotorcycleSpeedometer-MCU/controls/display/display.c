...
static void display_halInitMemoryBus(void);
...
static void display_halInitDma(void) {
	...
	_dmaHandle.Init = (DMA_InitTypeDef) {
	...
	_dmaHandle.XferCpltCallback = &display_lvglFlushCompleteCallback;
...
static void display_halInitMemoryBus(void) {
	...
	sramHandle.Instance = FMC_Bank1;
	sramHandle.Extended= FMC_Bank1E;
	sramHandle.Init = (FMC_NORSRAM_InitTypeDef) {
		...
		.WaitSignalActive = FMC_WAIT_SIGNAL_DISABLE,
	...
	norHandle.Instance = FMC_Bank1;
	norHandle.Extended= FMC_Bank1E;
	norHandle.Init = (FMC_NORSRAM_InitTypeDef) {
